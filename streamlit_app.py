...
# ------------------- Hilfsfunktionen -------------------
if "last_hue" not in st.session_state:
    st.session_state.last_hue = 0.0

def get_random_color():
    hue = st.session_state.last_hue
    hue = (hue + 0.618033988749895) % 1
    st.session_state.last_hue = hue
    s, v = 0.9, 0.9
...
if auth_status:
    ...
    def get_pdf_image(source, width=None, max_height=None):
    ...
    def generate_pdf(
        ...
        class RotatedImage(Flowable):
            def __init__(self, src, width, height, angle=90):
                super().__init__()
                self.src = src
                self.width = width
                self.height = height
            ...
            def draw(self):
                c = self.canv
                c.saveState()
                c.translate(self.height, 0)
                c.rotate(self.angle)
                c.drawImage(self.src, 0, 0, width=self.width, height=self.height)
                c.restoreState()
        ...
        for cont in containers:
            ...
            top_lbl = Paragraph("Topview", door_style)
            tv_h = tv_img.drawHeight
            sv_lbl_top = Paragraph("Sideview", door_style)
            sv_img = RotatedImage(sideview_images[num - 1], width=tv_h, height=img_w)

            left_col = Table(
                [
                    [top_lbl, None, sv_lbl_top],
                    [tv_img, Spacer(12, 1), sv_img],

                    [Paragraph("Door", door_style), None, Paragraph("Door", door_style)]
                ],
                colWidths=[img_w, 12, img_w],
                rowHeights=[None, tv_h, None]
            )
            left_col.setStyle(TableStyle([
                ...
                ('TOPPADDING', (0, 2), (-1, 2), 5 * mm),
            ]))

            # Belt table
            headers = [" ", "Nr.", "Specification", "Length (m)", "Width (mm)", "Roll Diameter (m)", "Weight/roll (kg)"]
            data = [headers] + [[
            ...
            box_tbl = Table(data, colWidths=[18 if idx == 0 else None for idx in range(len(headers))])
